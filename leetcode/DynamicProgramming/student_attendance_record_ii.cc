...
class Solution {
 public:
  ...
  void GenerateReport(int report, const int n, int late_count,
                      int absent_count) {
    // 结束条件
    if (report >= n) {
...
class Solution1 {
 public:
  ...
  int GenerateReport(int report, const int n, int late_count, int absent_count,
 ...
 public:
  int checkRecord(int n) {
    vector<vector<vector<long>>> dp(
        n, vector<vector<long>>(2, vector<long>(3, 0)));
    dp[0][0][0] = 1;
    dp[0][1][0] = 1;
    dp[0][0][1] = 1;
